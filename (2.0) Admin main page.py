...
def login_page():
    ...
    LogPg_ad_button.configure(command=lambda:refresh_page(Login_page_window, "admin_main_page"))
    ...
    LogPg_settings_button.configure(command=lambda:refresh_page(Login_page_window, "settings"))
...
def settings_page():
    ...
    SetPg_return_button.configure(command=lambda: refresh_page(settings_page_window, "login"))
...
def confirm_color_button(SetPg_theme_dropdownmenu, SetPg_appearance_dropdownmenu, settings_page_window):
    ...
    refresh_page(settings_page_window, "settings")

def admin_main_page():
    Admin_main_window, admin_main_frame = create_window("Admin Main Page")

    # Create the tab system within the admin main page
